...
def tempo_plot(X,Y):
    ...
    class1_x, class1_y = np.split(class1,2,axis=1)
    class2_x, class2_y = np.split(class2,2,axis=1)
    plt.xlabel("input1")
    plt.ylabel("input2")
    plt.plot(class1_x, class1_y, "o")
    plt.plot(class2_x, class2_y, "x")
    plt.show()
...
if __name__ == "__main__":
    ...
    is_feature = False
    feature = PolynomialFeatures(2) if is_feature == True else None
    ...
    for i in range(0,5):
        ...
        #tempo_plot(train_x_data, train_y_data)
        ...
        if synthetic is True:
            ...
            plot(train_x_data, np.squeeze(train_y_data), test_x_data, test_y_data, x1_test, x2_test, y_hat_plot, "./Results/tree_result_{}".format(i))
